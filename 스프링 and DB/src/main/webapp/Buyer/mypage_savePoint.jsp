...
<link href="<c:url value='/resources/css/Common/sub_main.css?after'/>"
	rel="stylesheet" />
<link
	href="<c:url value='/resources/css/Buyer/mypage_savePoint.css?after'/>"
	rel="stylesheet" />
<title>적립금</title>
</head>
<body>
						...
						<article id="myPage">
							...
							<div class="save_point--point">
								...
								<div class="point-explain">
									<p>- 적립금은 구매 확정 시 지급됩니다. (별도의 구매 확정이 없더라도 상품 발송 후 9일이 지난
										경우에는 자동 구매 확정 됩니다.)</p>
								...
								<div class="history--option">
									<c:choose>
										<c:when test="${condition eq '적립'}">
											...
											<a href="BuyerMyPageSavePoint.by?status=적립" id="option-save"
												class="active">적립</a>
										...
										<c:when test="${condition eq '사용'}">
											...
											<a href="BuyerMyPageSavePoint.by?status=사용" id="option-use"
												class="active">사용</a>
										</c:when>
										<c:otherwise>
											<a href="BuyerMyPageSavePoint.by?status=전체" id="option-total"
												class="active">전체</a>
											<a href="BuyerMyPageSavePoint.by?status=적립" id="option-save">적립</a>
											<a href="BuyerMyPageSavePoint.by?status=사용" id="option-use">사용</a>
										</c:otherwise>
											...
											<c:choose>
												<c:when test="${point.status eq '적립'}">
													<tr>
														...
														<td class="sp-content left">${point.contentTitle}<span
															class="enter content-detail"><a href="#a">${point.contentDetail}</a></span></td>
														<td class="sp-orderNum"><a
															href="OrderResearch.or?order_id=${point.orderId}">${point.orderId}</a></td>
												...
												<c:otherwise>
													<tr>
														...
														<td class="sp-content left">${point.contentTitle}<span
															class="enter content-detail">${point.contentDetail}</span></td>
														<td class="sp-orderNum"><a
															href="OrderResearch.or?order_id=${point.orderId}">${point.orderId}</a></td>
														<td class="sp-appDate">${point.applicationDate}</td>
													</tr>
								...
								<div class="n-paging">
									<ul>
										<c:if test="${pageMaker.prev}">
											<li><a
												href="BuyerMyPageSavePoint.by${pageMaker.makeQuery(pageMaker.startPage - 1,condition)}"
												class="prev">이전</a></li>
										</c:if>
										...
										<c:forEach begin="${pageMaker.startPage}"
											...
											<c:if test="${currentPage eq idx}">
												<li><a
													href="BuyerMyPageSavePoint.by${pageMaker.makeQuery(idx,condition)}"
										...
										<c:if test="${pageMaker.next && pageMaker.endPage> 0}">
											<li><a
												href="BuyerMyPageSavePoint.by${pageMaker.makeQuery(pageMaker.endPage + 1,condition)}"
												class="next">다음</a></li>
										</c:if>
									</ul>
								</div>
